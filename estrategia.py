...
def get_estategia(ticker, startDate, endDate, sht, lng):
    initial_amount = 100000
    taxa_corretagem = 11
    fixed_fee = 0.00031302
    ...
    stocks = yf.download(ticker, start=startDate, end=endDate)
    # stocks['Sht'] = stocks['Adj Close'].rolling(sht).mean()
    stocks['Lng'] = stocks['Adj Close'].rolling(lng).mean()
    stocks['Sht'] = stocks['Adj Close'].ewm(span=sht).mean()
    # stocks['Lng'] = stocks['Adj Close'].ewm(span=lng).mean()

    parameters = str(sht) + ' - ' + str(lng)
